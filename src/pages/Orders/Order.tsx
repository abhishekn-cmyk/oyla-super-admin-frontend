...
// const StatusCards = ({ stats, analyticsData }: { stats: IOrderStats | undefined, analyticsData: any }) => {
...
//         const statusProfit = analyticsData?.profitByStatus?.[status];
//         const profitPercentage = statusProfit && statusProfit.revenue > 0 ? 
//           (statusProfit.profit / statusProfit.revenue) * 100 : 0;
...
//     // Priority: meal.costPrice > product.costPrice
...
//   // Calculate profit percentage for individual meal
//   const getMealProfitPercentage = (meal: IMeal): number => {
...
//   // Enhanced order calculations with percentages
...
//   const getProfitPercentage = (order: IOrder): number => {
...
//     return order.meals?.reduce((total, meal) => total + (meal.quantity || 1), 0) || 0;
//   };
...
//     // Calculate average profit percentage per order
//     const totalProfitPercentage = orders.reduce((sum, order) => sum + getProfitPercentage(order), 0);
//     const averageProfitPercentage = orders.length > 0 ? totalProfitPercentage / orders.length : 0;
...
//       const status = order.orderStatus || 'unknown';
//       if (!acc[status]) {
//         acc[status] = { revenue: 0, cost: 0, profit: 0, count: 0 };
//       }
//       acc[status].revenue += getTotalPrice(order);
//       acc[status].cost += getTotalCost(order);
//       acc[status].profit += getProfit(order);
//       acc[status].count += 1;
//       return acc;
//     }, {} as Record<string, { revenue: number; cost: number; profit: number; count: number }>);
...
//       const profit = getProfit(order);
//       const profitPercentage = getProfitPercentage(order);
...
//         "Total Revenue": formatCurrency(totalPrice),
//         "Total Cost": formatCurrency(totalCost),
//         "Profit": formatCurrency(profit),
//         "Profit Margin": `${profitPercentage >= 0 ? '+' : ''}${profitPercentage.toFixed(1)}%`,
...
//                   desc: `Net profit (${analyticsData.overallProfitPercentage.toFixed(1)}%)`, 
//                   formatter: formatCurrency
//                 },
//                 { 
//                   title: "Avg. Margin", 
//                   value: analyticsData.averageProfitPercentage, 
//                   icon: <BarChart3 className="w-6 h-6 text-orange-600"/>, 
//                   color: "bg-orange-50 border-orange-200", 
//                   desc: "Average profit margin", 
...
//               <StatusCards stats={stats} analyticsData={analyticsData} />
//             </div>
//           </div>
//         )}
...
//                     <tr>
//                       {["Order ID", "Customer", "Items", "Total", "Profit", "Margin %", "Status", "Payment", "Date", "Actions"].map(col => (
//                         <th
//                           key={col}
...
//                     {analyticsData.overallProfitPercentage.toFixed(1)}% margin
//                   </p>
//                 </div>
...
//                   <p className="text-2xl font-bold text-gray-900 mt-2">
//                     {analyticsData.averageProfitPercentage.toFixed(1)}%
//                   </p>
...
//               {/* Order Status with Profit */}
//               <div className="bg-gray-50 rounded-xl p-6">
//                 <h3 className="text-lg font-semibold mb-4">Order Status Profit Analysis</h3>
//                 <div className="space-y-3">
//                   {Object.entries(analyticsData.profitByStatus).map(([status, data]: [string, any]) => {
//                     const percentage = analyticsData.totalOrders > 0 ? 
//                       ((data.count / analyticsData.totalOrders) * 100) : 0;
...
//                     const percentage = analyticsData.totalOrders > 0 ? 
//                       ((count as number / analyticsData.totalOrders) * 100) : 0;
...
//                       }`}>
//                         {formatCurrency(getProfit(selectedOrder))} ({getProfitPercentage(selectedOrder) >= 0 ? '+' : ''}{getProfitPercentage(selectedOrder).toFixed(1)}%)
//                       </p>
//                     </div>
//                   </div>
//                 </div>
...
//                           <th className="px-3 py-2 text-left font-medium text-gray-700">Margin %</th>
...
//                           const mealPrice = getMealPrice(meal);
//                           const mealCost = getMealCost(meal);
//                           const mealProfitPercentage = getMealProfitPercentage(meal);
...
//                               <td className="px-3 py-2">
//                                 <span className={mealProfitPercentage >= 0 ? 'text-green-600' : 'text-red-600'}>
//                                   {mealProfitPercentage >= 0 ? '+' : ''}{mealProfitPercentage.toFixed(1)}%
//                                 </span>
//                               </td>
...
function OrdersContent() {
  ...
  const getTotalPrice = (order: IOrder): number => {
    ...
    // Regular orders - calculate from meals
    return order.meals?.reduce((sum, meal) => {
      const mealPrice = getMealPrice(meal);
      const quantity = meal.quantity || 1;
      return sum + (mealPrice * quantity);
  ...
  const getTotalCost = (order: IOrder): number => {
    ...
    // Calculate from meals' cost prices
    return order.meals?.reduce((sum, meal) => {
      const mealCost = getMealCost(meal);
      const quantity = meal.quantity || 1;
      return sum + (mealCost * quantity);
  ...
  const getProfit = (order: IOrder): number => {
    // If profit is already calculated on server, use it
    if (order.profit !== undefined && order.profit !== 0) {
      return order.profit;
    }
    
    // Calculate profit: Total Price - Total Cost
    const totalPrice = getTotalPrice(order);
    const totalCost = getTotalCost(order);
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {isTableLoading ? (
                      ...
                      currentOrders.map((order) => {
                        ...
                        // CORRECTED: Calculate profit for each order individually
                        const totalPrice = getTotalPrice(order);
                        const totalCost = getTotalCost(order);
